...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    w2, h2 = img.shape[0] // 2, img.shape[1] // 2
    img2 = cv2.resize(img, (w2, h2), interpolation=cv2.INTER_CUBIC)
...
if __name__ == "__main__":
    ...
    ref_path = "/Users/zihua.zeng/Dataset/al_sampler_0530_edz/validation/Normal"
    dis_path = "/Users/zihua.zeng/Dataset/al_sampler_0530_edz/validation/Uneven"

    niqe_scores = []
    for fn in tqdm(os.listdir(dis_path)):
        if not fn.endswith(".jpg"):
            continue

        dummy = cv2.imread(os.path.join(dis_path, fn))
        h, w, c = dummy.shape
        if h < 500 or w < 500:
            continue

        ref = np.array(Image.open(os.path.join(dis_path, fn)).convert('LA'))[:, :, 0]  # ref
        niqe_scores.append(niqe(ref))

    plt.hist(niqe_scores)
    plt.show()
